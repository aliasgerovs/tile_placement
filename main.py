...
def csp_solver(current_index, current_sum, path, functions, Landscape):
    ...
    if current_index == len(Landscape):
        ...
        for func in functions:
            ...
            result = csp_solver(current_index + 1, current_sum_total, path, functions, Landscape)
            if result:
                print(f' The index : {current_index}, Area : {Landscape[current_index]}, Applied Shape : {func.__name__}, Applied shape result : {current_sum_total} \n') 
                return True
            else:
                path.pop()
...
result = csp_solver(0, {1:0, 2:0, 3:0, 4:0}, path, functions, Landscape)
print( 'Tiles Used Counts :' ,path.count('L_shape'), path.count('Full_block'),path.count('Outer_block'))
print('\n')
if result:
